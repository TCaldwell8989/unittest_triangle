import unittest
from triangle import triangle_math, triangle_facts
...
class TestTriangle(unittest.TestCase):
    ...
    def test_triangle_area_with_negative_input(self):
    ...
    def test_right_angle_triangle(self):
        ...
        self.assertTrue(triangle_math.is_right_angle(1.22222222222, 6.6666666666, 6.7777777777))
    ...
    def test_right_angle_triangle_with_null_and_negative_input(self):

        with self.assertRaises(ValueError):
            triangle_math.is_right_angle(-1, 2, 3)
        ...
        # Test for null lengths
        with self.assertRaises(ValueError):
            triangle_math.is_right_angle(0, 1, 1)

        with self.assertRaises(ValueError):
            triangle_math.is_right_angle(1, 0, 1)

        with self.assertRaises(ValueError):
            triangle_math.is_right_angle(1, 1, 0)

    def test_random_fact(self):
        facts = ['1']
        self.assertTrue(triangle_facts.random_fact(facts))

    def test_random_fact_with_empty_string(self):
        with self.assertRaises(ValueError):
            facts = ['']
            triangle_facts.random_fact(facts)
